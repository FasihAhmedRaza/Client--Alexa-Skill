...
class LaunchRequestHandler(AbstractRequestHandler):
    ...
    # type: (HandlerInput) -> Response
        tired_entity_synonyms = EntityValueAndSynonyms(
            value="tired",
            synonyms=["dog-tired" , "weary" , "sleepy"]
            )
        tired_entity = Entity(id="tir", name=tired_entity_synonyms)

        energetic_entity_synonyms = EntityValueAndSynonyms(
            value="energetic",
            synonyms=["active" , "animated" , "lively"]
        )
        energetic_entity = Entity(id="ener", name=energetic_entity_synonyms)

        passive_entity_synonyms = EntityValueAndSynonyms(
            value="passive",
            synonyms=["obedient" , "obedient" , "compliant"],
        )
        passive_entity = Entity(id="pass", name=passive_entity_synonyms)

        hyper_entity_synonyms = EntityValueAndSynonyms(
            value="hyper",
            synonyms=[" excitable" ," jumpy" ,"jittery" ," nervous"]
            )
        hyper_entity = Entity(id="hyp", name=hyper_entity_synonyms)
        ...
        replace_entity_directive = DynamicEntitiesDirective(
            ...
            types=[
                EntityListItem(
                    name="energyType",
                    values=[tired_entity, energetic_entity, passive_entity, hyper_entity],
                )
            ],
        )

        logger.info("In LaunchRequestHandler")
        speech = ('hi , Welcome to pet match. I can help you find the best dog '
